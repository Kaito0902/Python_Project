...
class LogController:
    ...
    def ghi_nhat_ky(self, hanh_dong):
        """
        Ghi nhật ký hành động hiện tại của người dùng đang đăng nhập.
        :param hanh_dong: Mô tả hành động.
        """
        if "ma_nguoi_dung" in current_user:
            ma_nguoi_dung = current_user["ma_nguoi_dung"]
            self.log_model.ghi_nhat_ky(ma_nguoi_dung, hanh_dong)
        else:
            raise Exception("Không có người dùng đang đăng nhập để ghi nhật ký.")
