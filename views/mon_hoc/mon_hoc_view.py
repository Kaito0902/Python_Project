...
class MonHocFrame(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        # Nút thao tác: Thêm, Sửa, Xóa chỉ được hiển thị nếu có quyền tương ứng theo module "mon_hoc"
        btn_frame = ctk.CTkFrame(search_frame, fg_color="white")
        btn_frame.pack(side="right")

        if self.user_permissions.get("mon_hoc", {}).get("them"):
            ctk.CTkButton(
                btn_frame,
                fg_color="#4CAF50",
                text="Thêm",
                ...
                command=self.them_mon_hoc,
                width=80
            ).pack(side="left", padx=5, pady=20)

        if self.user_permissions.get("mon_hoc", {}).get("sua"):
            ctk.CTkButton(
                btn_frame,
                fg_color="#fbbc0e",
                text="Sửa",
                text_color="white",
                font=("Verdana", 13, "bold"),
                command=self.sua_mon_hoc,
                width=80
            ).pack(side="left", padx=5, pady=20)

        if self.user_permissions.get("mon_hoc", {}).get("xoa"):
            ctk.CTkButton(
                btn_frame,
                fg_color="#F44336",
                text="Xóa",
