...
class DiemView(ctk.CTkFrame):
    ...
    def _create_ui(self):
        ...
        header.pack(fill="x")
        ctk.CTkLabel(header, text="Nhận Diện Điểm & Thống Kê", font=("Verdana", 18, "bold"), text_color="white").pack(
            pady=10)

        # Load ảnh & lưu
        top = ctk.CTkFrame(self, fg_color="#ffffff")
        top.pack(fill="x", padx=20, pady=10)
        ctk.CTkButton(top, text="Load Ảnh & Lưu", fg_color="#3084ee", hover_color="#2a6bcc",
                      command=self._load_and_save).pack()
