...
class ThemLopHoc(ctk.CTkToplevel):
    ...
    def xac_nhan(self):
        ...
        try:
            success, message = self.controller.add_class(ma_lop, ma_mon, int(so_luong), int(hoc_ky), int(nam), None)
            self.attributes('-topmost', False)
            if success:
            ...
            self.attributes('-topmost', False)
            messagebox.showerror("Lỗi", f"Lỗi khi thêm lớp học: {str(e)}")
            self.attributes('-topmost', True)
