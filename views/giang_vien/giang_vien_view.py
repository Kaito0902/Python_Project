...
class GiangVienFrame(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        icon = ctk.CTkImage(
            Image.open(r"D:\Downloads\sever nro\icon\Python_Project-master1\resources\images\search.png").resize((20, 20)),
            size=(20, 20)
        )
        ...
        # Nút thao tác – chỉ hiển thị nếu quyền được cấp theo thuộc tính được kiểm tra qua hàm has_permission
        btn_frame = ctk.CTkFrame(search_frame, fg_color="white")
        btn_frame.pack(side="right")

        if self.user_permissions.get("giang_vien", {}).get("them"):
            ctk.CTkButton(
                btn_frame,
                fg_color="#4CAF50",
                text="Thêm",
                ...
                text_color="white",
                command=self.them_giang_vien,
                width=80
            ).pack(side="left", padx=5, pady=20)
        if self.user_permissions.get("giang_vien", {}).get("sua"):
            ctk.CTkButton(
                btn_frame,
                fg_color="#fbbc0e",
                text="Sửa",
                font=("Verdana", 13, "bold"),
                text_color="white",
                command=self.sua_giang_vien,
                width=80
            ).pack(side="left", padx=5, pady=20)
        if self.user_permissions.get("giang_vien", {}).get("xoa"):
            ctk.CTkButton(
                btn_frame,
                fg_color="#F44336",
                text="Xóa",
