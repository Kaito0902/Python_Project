...
class MainView(ctk.CTk):
    def __init__(self):
        ...
        # 🔥 Lấy danh sách quyền từ database dựa trên vai trò sử dụng key "vai_tro_id"
    ...
    def show_permission_frame(self):
        for widget in self.main_content.winfo_children():
            widget.destroy()
        permission_frame = PermissionView(self.main_content)
        permission_frame.pack(fill="both", expand=True)
    ...
    # Xóa hết nội dung khung chính hiện có
        for widget in self.main_content.winfo_children():
            widget.destroy()
        # Tạo và hiển thị giao diện dành cho admin
        class_admin_frame = LopAdminFrame(self.main_content, app=self)
        class_admin_frame.pack(fill="both", expand=True)


    def show_classAdmin_detail_frame(self):
        for widget in self.main_content.winfo_children():
            widget.destroy()
        detail_frame = ChiTietLopAdminFrame(self.main_content, app=self)
        detail_frame.pack(fill="both", expand=True)
        detail_frame.thong_tin_lop(self.selected_ma_lop)

    def show_classGV_detail_frame(self):
        for widget in self.main_content.winfo_children():
            widget.destroy()
        detail_frame = ChiTietLopGiangVienFrame(self.main_content, app=self)
        detail_frame.pack(fill="both", expand=True)

    def show_diemlop_frame(self):
        for widget in self.main_content.winfo_children():
            widget.destroy()
        bang_diem_frame = QuanLyLopTabbedPane(self.main_content, self.selected_ma_lop)
        bang_diem_frame.pack(fill="both", expand=True)
